...
pub struct ModelInfo {
    ...
    /// `duplicate field`. The wire value is produced by
    /// `get_model_instructions()` on the way out and promoted into
    /// `instructions_template` on the way in, so nothing is lost by keeping this
    /// one runtime-only.
    #[serde(skip)]
